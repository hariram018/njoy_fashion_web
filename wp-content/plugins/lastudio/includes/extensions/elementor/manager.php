...
add_action('plugins_loaded', function (){
	if(empty(get_option('lastudio_has_elementor_error', false))){
		if( ($controls_stack = LASTUDIO_PLUGIN_PATH . 'includes/extensions/elementor/override/includes/base/controls-stack.php') && file_exists($controls_stack) ) {
			require_once $controls_stack;
		}
		if( ($css_base = LASTUDIO_PLUGIN_PATH . 'includes/extensions/elementor/override/core/files/css/base.php') && file_exists($css_base) ) {
		...
		if ( version_compare( ELEMENTOR_VERSION, '3.2.0', '<' ) ) {
			if( ($responsive = LASTUDIO_PLUGIN_PATH . 'includes/extensions/elementor/override/old/core/responsive/responsive.php') && file_exists($responsive) ) {
				require_once $responsive;
			}
		}
		else{
			if( ($breakpoints = LASTUDIO_PLUGIN_PATH . 'includes/extensions/elementor/override/core/breakpoints/manager.php') && file_exists($breakpoints) ) {
