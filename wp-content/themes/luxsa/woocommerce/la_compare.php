...
<div id="la_compare_table_wrapper">
    ...
    $lists = Luxsa_WooCommerce_Compare::get_data();

    $default_attr = luxsa_get_wc_attribute_for_compare();
    $tax_attr = luxsa_get_wc_attribute_taxonomies();
    $all_attr = array_merge($default_attr, $tax_attr);

    $attribute_allow = luxsa_get_option('compare_attribute', $default_attr);
    ...
    add_filter('luxsa/filter/enable_image_lazyload', '__return_false');
    ...
    if (!empty($lists)) {
        foreach ($lists as $product_id) {
            ...
            $term['id'] = $product_id;
            $term['sku'] = ($_product->get_sku() ? $_product->get_sku() : esc_html__('N/A', 'luxsa'));
        ...
        ?>
        <table class="la-compare-table la-compare-table-items <?php if($total_product > 3){ echo 'show-vtb'; } ?> ">
            <tbody>
            <?php
            ...
            foreach( $attribute_allow as $attr_alow ){
                if(in_array($attr_alow, array('image','title','add-to-cart'))){
                    $label_list['info'] = esc_html_x('Product Info', 'front-view', 'luxsa');
                }
                else{
                ...
                foreach ($terms as $item) {
                    ...
                    elseif ($k == 'stock') {
                        ...
                        if ($item['availability'] && !empty($item['availability']['availability'])) {
                            ...
                            if ($item['stock']) {
                                $text_stock = esc_html__('In stock', 'luxsa');
                                $class_stock = 'in-stock';
                            } else {
                                $text_stock = esc_html__('Out of stock', 'luxsa');
        ...
        <table class="la-compare-table la-compare-table-heading <?php if($total_product > 3){ echo 'show-vtb'; } ?>">
            <tbody>
            <?php
            foreach ($label_list as $k => $v) {
                echo '<tr class="compare-tr compare-tr-'. esc_attr($k) .'">';
        ...
        ?>
        <p class="text-center"><?php esc_html_e('No products were added to the list', 'luxsa') ?></p>
        <?php
    }
    ?>
</div>
